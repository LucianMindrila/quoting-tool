...
export default function MatPicker({ open, onClose, onSelect }) {
  ...
  function renderEgger() {
          ...
          {Object.entries(grouped).map(([grp, grpItems]) => (
            <div key={grp}>
              <div className="picker-group-label">
                Group {grp}
                <span className="grp-price">£{EGGER_GRP_PRICE[grp]}/sht</span>
              </div>
              {grpItems.map(item => (
    ...
    if (subTab === 'ps-gloss') return renderSimpleList(PS_GLOSS_DECORS.map(d => ({ matId:'eg-'+d.id, code:d.code, name:d.name, price:'£182/sht' })));
    if (subTab === 'ps-matt')  return renderSimpleList(PS_MATT_DECORS.map(d => ({ matId:'eg-'+d.id, code:d.code, name:d.name, price:'£194/sht' })));
    if (subTab === 'tm') {
      const byType = {};
      TM_DECORS.forEach(d => {
        if (!byType[d.tmtype]) byType[d.tmtype] = [];
        byType[d.tmtype].push({ matId:'eg-'+d.id, code:d.code, name:d.name, price:`£${d.price}/sht` });
      });
      return (
        <>
  ...
  function renderMdf() {
    return renderSimpleList(MDF_IDS.map(id => ({
      ...
      name:  '',
      price: MATERIALS[id].price ? `£${MATERIALS[id].price}/sht` : undefined,
      cut:   `Cut: £${MATERIALS[id].cutCost}/sht`,
    })));
  }
