...
function resolveMatId(rawName) {
  ...
  // 7. Base decor code match — handles wrong texture suffix (e.g. "W980 ST2" → matches "W980 ST7")
  //    Extracts the leading letter+digits code (W980, H3382, U961, F187, etc.)
  const baseMatch = rawName.trim().match(/^([A-Za-z]\d{3,4})/);
  if (baseMatch) {
    const base = norm(baseMatch[1]);
    for (const [id, mat] of entries) {
      if (mat._code && norm(mat._code).startsWith(base)) return id;
    }
  }
