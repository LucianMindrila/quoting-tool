...
function geneticAlgorithm(items, AW, AH, popSize, generations) {
  ...
  let popScores = pop.map(p => guillotinePack(p, AW, AH));
  let bestIdx   = popScores.indexOf(Math.min(...popScores));
  let best      = popScores[bestIdx];
  let bestOrder = [...pop[bestIdx]];
  if (best === 1) return { sheets: 1, order: bestOrder };

  for (let gen = 0; gen < generations; gen++) {
    popScores = pop.map(p => guillotinePack(p, AW, AH));
    const curMin    = Math.min(...popScores);
    const curBestIdx = popScores.indexOf(curMin);
    if (curMin < best) { best = curMin; bestOrder = [...pop[curBestIdx]]; }
    if (best === 1) break;

    const ranked = popScores.map((s,i) => ({s,i})).sort((a,b) => a.s-b.s);
    const newPop = [pop[ranked[0].i], pop[ranked[1].i]];

    while (newPop.length < popSize) {
      const pa    = gaTournament(pop, popScores);
      const pb    = gaTournament(pop, popScores);
...
function branchAndBound(items, AW, AH) {
  ...
  if (best === lowerBound) return { sheets: best, order: bestOrder };
  ...
  function permute(arr, l) {
    ...
    if (l === arr.length) {
      const order = arr.map(i => items[i]);
      const s = guillotinePack(order, AW, AH);
      if (s < best) { best = s; bestOrder = order; }
      return;
    }
...
// Single source of truth: finds the best packing order using all algorithms,
// then records placements from that same order — sheet count and diagram always agree.
export function optimiseLayout(pieces, mat) {
  ...
  if (!items.length) return { sheets: 0, layout: [] };
  const n = items.length;

  // Seed with area-descending
  let bestOrder = [...items].sort((a, b) => b.area - a.area);
  let best = guillotinePack(bestOrder, AW, AH);
  if (best === 1) { const layout = gpPackLayout(bestOrder, AW, AH); return { sheets: layout.length, layout }; }

  // Branch & Bound (exhaustive) for small jobs — guaranteed optimal
  if (n <= 7) {
    const bbRes = branchAndBound(items, AW, AH);
    if (bbRes.sheets < best) { best = bbRes.sheets; bestOrder = bbRes.order; }
    if (best === 1) { const layout = gpPackLayout(bestOrder, AW, AH); return { sheets: layout.length, layout }; }
  }

  // Simulated Annealing — explores thousands of orderings
  if (n > 7) {
    const saRes = simulatedAnnealing(items, AW, AH);
    if (saRes.sheets < best) { best = saRes.sheets; bestOrder = saRes.order; }
    if (best === 1) { const layout = gpPackLayout(bestOrder, AW, AH); return { sheets: layout.length, layout }; }
  }

  // Genetic Algorithm for medium-sized jobs
  if (n > 7 && n <= 100) {
    const gaRes = geneticAlgorithm(items, AW, AH, 12, 40);
    if (gaRes.sheets < best) { best = gaRes.sheets; bestOrder = gaRes.order; }
    if (best === 1) { const layout = gpPackLayout(bestOrder, AW, AH); return { sheets: layout.length, layout }; }
  }

  // GRASP random restarts — tracks best ordering found
  ...
  for (let r = 0; r < graspRuns; r++) {
    ...
    if (s < best) { best = s; bestOrder = rand; }
    if (best === 1) break;
  }

  // Record actual placements from the best ordering found — layout.length === best
  const layout = gpPackLayout(bestOrder, AW, AH);
  return { sheets: layout.length, layout };
}

// ── MASTER OPTIMIZER ────────────────────────────────────────────
// Delegates to optimiseLayout so pricing and diagram always use the same result.
export function optimiseSheets(pieces, mat) {
  return optimiseLayout(pieces, mat).sheets;
}
