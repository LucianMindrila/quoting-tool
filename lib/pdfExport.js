...
export async function generateInvoicePDF({
  ...
  const fmt        = v  => `£${Number(v).toFixed(2)}`;
  const delivery   = grandDelivery ?? 0;
  const grandTotal = (grandMat ?? 0) + (grandCut ?? 0) + (grandEdge ?? 0) + delivery;
  ...
  if (delivery > 0) {
    tableBody.push(['Delivery', 'Flat rate within 30 miles', fmt(delivery)]);
  }
  ...
  const totalRows = [
    ...
    ['Cutting',   fmt(grandCut)],
    ['Edging',    fmt(grandEdge)],
    ...(delivery > 0 ? [['Delivery', fmt(delivery)]] : []),
  ...
  // Last day of the fulfilment month — e.g. delivery on 02 Jun → due 30 Jun
  let dueStr = 'Upon collection / delivery';
  if (fulfilment?.isoDate) {
    const [fy, fm] = fulfilment.isoDate.split('-').map(Number);
    const lastDay  = new Date(fy, fm, 0); // day 0 of next month = last day of this month
    dueStr = lastDay.toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
  }
